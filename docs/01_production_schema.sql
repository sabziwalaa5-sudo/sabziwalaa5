-- SQL Migration Script for Sabjiwala 5 (Production-Grade Schema)

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Drop old types and tables if they exist to start fresh
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS public.handle_new_user();
DROP FUNCTION IF EXISTS get_nearby_vendors(double precision, double precision, double precision);
...
CREATE TABLE IF NOT EXISTS public.addresses (
  ...
  tag TEXT DEFAULT 'Home' NOT NULL, -- 'Home', 'Office', 'Other'
...
CREATE TABLE IF NOT EXISTS public.orders (
  id TEXT PRIMARY KEY, -- Formatted: e.g. SBJ50001
...
CREATE TABLE IF NOT EXISTS public.reward_transactions (
  ...
  transaction_type TEXT NOT NULL, -- 'EARNED', 'REDEEMED', 'WELCOME_BONUS', 'REFERRAL_BONUS', 'FESTIVAL_BONUS', 'CANCEL_DEDUCTION', 'MANUAL_ADJUSTMENT'
  campaign_name TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 15. Reward Campaigns Table
CREATE TABLE IF NOT EXISTS public.reward_campaigns (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  bonus_points INTEGER NOT NULL CHECK (bonus_points > 0),
  campaign_type TEXT NOT NULL, -- 'WELCOME', 'REFERRAL', 'FESTIVAL', 'BIRTHDAY'
...
CREATE TABLE IF NOT EXISTS public.coupons (
  ...
  discount_type TEXT DEFAULT 'percentage' NOT NULL, -- 'percentage', 'fixed'
...
-- 20. Delivery Zones Table
CREATE TABLE IF NOT EXISTS public.delivery_zones (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL UNIQUE,
  zone_type TEXT DEFAULT 'circle' NOT NULL, -- 'circle', 'polygon'
  center_lat DECIMAL(9, 6),
  center_lng DECIMAL(9, 6),
  radius_meters DECIMAL(10, 2), -- for circle zones
  polygon_coordinates JSONB, -- geojson polygon paths
...
CREATE TABLE IF NOT EXISTS public.audit_logs (
  ...
  action TEXT NOT NULL, -- e.g. 'UPDATE_PRODUCT', 'DELETE_ZONE'
...
-- Setup Indexing on highly-queried foreign keys/fields
...
-- Trigger for Auto Profile and Wallet Creation on signup
...
-- Trigger for Auto-deducting Inventory on Order insertion
...
-- Trigger for Reward credits upon status update to 'DELIVERED'
CREATE OR REPLACE FUNCTION public.credit_rewards_on_delivery()
RETURNS TRIGGER AS $$
DECLARE
  earned_pts INTEGER;
  point_val DECIMAL(10,2);
  current_bal INTEGER;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
